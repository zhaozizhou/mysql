...
from prometheus_client import Counter,CollectorRegistry, Gauge, push_to_gateway

#c=Counter('test','测试', ['method', 'endpoint'], registry=registry)
#c.inc()


#registry = CollectorRegistry()
#g = Gauge('ping', '检测最大响应时间',['dst_ip','city'], registry=registry) #Guage(metric_name,HELP,labels_name,registry=registry)
#g.labels('192.168.1.6','shenzhen').set(results[1]) #set设定值
#g.labels('192.168.1.6','shenzhen').dec(2)  #dec递减2
#g.labels('192.168.1.6','shenzhen').inc()  #inc递增，默认增1
#push_to_gateway('192.168.21.128:9091', job='ping_status', registry=registry)
...
def format_tcp_result(results):
    ...
    if results[0]:
        registry = CollectorRegistry()
        g = Gauge('ping', '检测最大响应时间',['dst_ip','city'], registry=registry) #Guage(metric_name,HELP,labels_name,registry=registry)
        g.labels('192.168.1.6','6379').set(results[1]) #set设定值
        push_to_gateway('192.168.21.128:9091', job='ping_status', registry=registry)
...
if __name__ == "__main__":
    desc = '''--------------------------------------------------------------------------
test tcp port connet time.
--------------------------------------------------------------------------'''

    example_text = '''examples:
    tcping qq.com
    tcping qq.com -n 10 -p 443 -i 5 -w 1
    \n
'''
